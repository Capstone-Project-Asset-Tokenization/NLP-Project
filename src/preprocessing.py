...
class AmharicTextProcessor:
    ...
    def _get_short_forms(self):
        ...
        with open(self.expansion_file_dir, encoding='utf8') as file:
            ...
            for line in file:
                ...
                if line:
                    expanded = line.split("-")
                    expansions[expanded[0].strip()] = expanded[1].replace(" ", '_').strip()
            return expansions
    ...
    def collocation_finder(self, tokens, bigram_dir):
        ...
        finder.apply_freq_filter(3)  # Only consider bigrams that appear more than 3 times
        frequent_bigrams = finder.nbest(bigram_measures.chi_sq, 5)  # Top 5 bigrams based on chi-square measure
        ...
        if not os.path.exists(bigram_dir):
            ...
            if len(tokenized_sentence) == 1:
                ...
                while index <= len(tokenized_sentence) - 2:
                    multi_word = tokenized_sentence[index] + ' ' + tokenized_sentence[index + 1]
...
def preprocess_text(text):
    ...
    tokens = [pre_processor.remove_ascii_and_numbers(token) for token in tokens]
